...
/**
 * Correct Opportunity Redistribution Script
 * 
 * This script redistributes the data correctly:
...
async function redistributeOpportunitiesCorrectly() {
  try {
    console.log('🔄 Starting correct opportunity redistribution...');
    ...
    if (opportunityPeople.length > 0) {
      ...
      for (const [companyName, people] of Object.entries(companyGroups)) {
        ...
        let probability = 25;
        if (avgEngagementScore >= 12) probability = 75;
        else if (avgEngagementScore >= 10) probability = 50;
        else if (avgEngagementScore >= 8) probability = 35;
...
// Run the script
redistributeOpportunitiesCorrectly()
  .then(() => {
