/**
 * 🎯 EFFICIENT 5BARS BUYER GROUP ANALYSIS
 * 
 * 2025 Optimized approach:
 * 1. Get company by domain enrichment
 * 2. Search employees by company ID (not name)
 * 3. Validate current employment with active_experience
 */
...
class Efficient5BarsBuyerGroupAnalysis {
  constructor() {
  ...
  async execute() {
    console.log('🎯 EFFICIENT 5BARS BUYER GROUP ANALYSIS (2025)');
    console.log('===============================================');
    ...
    try {
      ...
      // Step 2: Enrich company by domain (most efficient)
  ...
  async enrichCompanyByDomain() {
    ...
    try {
      ...
      // Use the most efficient approach: company enrichment by website
  ...
  async searchCurrentEmployeesByCompanyId() {
    ...
    try {
      ...
      // Use the most efficient query: search by company ID with active experience
      const searchQuery = {
        query: {
          bool: {
            must: [
      ...
      // Collect detailed data for each current employee
      for (const employeeId of employeeIds.slice(0, 50)) { // Limit to first 50
        try {
          const personData = await this.collectPersonData(employeeId);
          
          if (personData) {
            this.results.currentEmployees.push({
      ...
      console.log(`   ✅ Collected detailed data for ${this.results.currentEmployees.length} current employees`);
  ...
  async updateDatabaseWithPeople() {
    console.log('\n💾 STEP 5: Updating database with current employees...');
  ...
  async saveResults() {
    ...
    const filename = `efficient-5bars-buyer-group-analysis-${timestamp}.json`;
...
async function main() {
  const analyzer = new Efficient5BarsBuyerGroupAnalysis();
...
module.exports = Efficient5BarsBuyerGroupAnalysis;
