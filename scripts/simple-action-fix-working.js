...
/**
 * Simple Action System Fix - Working Version
 * Fixes orphaned actions, lastAction, and nextAction without complex queries
 */
...
async function main() {
  console.log('🔧 SIMPLE ACTION SYSTEM FIX');
  console.log('============================\n');
  ...
  try {
      ...
      try {
        ...
        if (action.name) {
          const person = await prisma.people.findFirst({
            where: {
              OR: [
                { name: { contains: action.name, mode: 'insensitive' } },
    ...
    // Step 2: Update lastAction for people
    ...
    // Step 4: Generate simple nextAction recommendations
    ...
    const peopleWithoutNextAction = await prisma.people.findMany({
      ...
      take: 25 // Process in smaller batches
    });
      ...
      try {
        ...
        await prisma.people.update({
          ...
          data: {
            ...
            nextActionReasoning: `Based on last action: ${person.lastAction || 'No previous action'}`,
