...
/**
 * 🔍 ENRICH COMPANIES WITH PERPLEXITY (SONAR-PRO) - SIMPLE VERSION
 * 
 * Uses Perplexity to research companies and CoreSignal to find employees
 */
...
class EnrichCompaniesWithPerplexity {
  constructor() {
    ...
    this.perplexityApiKey = process.env.ANTHROPIC_API_KEY;
  ...
  async execute() {
    console.log('🔍 ENRICHING COMPANIES WITH PERPLEXITY');
    console.log('=====================================\n');
  ...
  async researchWithPerplexity(company) {
    try {
      const query = `Research company "${company.name}" and provide: website URL, industry, employee count, location. Be concise.`;
      ...
      console.log(`   🔍 Researched with Perplexity`);
