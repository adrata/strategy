/**
 * 🚀 EMPLOYEE SEARCH + COLLECT ENRICHMENT SYSTEM
 * 
 * Uses the correct two-step process:
 * 1. Search with employee_multi_source/search/es_dsl to get ID
 * 2. Collect with employee_multi_source/collect/{id} to get full data
 */
...
class EmployeeSearchCollectEnrichment {
  ...
  /**
   * 🚀 MAIN EXECUTION - Search and collect employee data
   */
  async enrichPersonWithSearchCollect(personName) {
    console.log('🔍 EMPLOYEE SEARCH + COLLECT ENRICHMENT');
    ...
    try {
      ...
      // Step 1: Search for employee ID
      const employeeId = await this.searchForEmployeeId(person);
      ...
      // Step 2: Collect full employee data
      const employeeData = await this.collectEmployeeData(employeeId);
      ...
      console.log(`✅ Collected complete employee data`);
      console.log(`   Name: ${employeeData.full_name}`);
      console.log(`   Email: ${employeeData.primary_professional_email}`);
      console.log(`   Company: ${employeeData.active_experience_company}`);
      ...
      // Step 3: Update person with complete data
      await this.updatePersonWithEmployeeData(person, employeeData);
  ...
  /**
   * Step 1: Search for employee ID using multiple strategies
   */
  async searchForEmployeeId(person) {
    console.log('\n🔍 STEP 1: SEARCHING FOR EMPLOYEE ID...');
    ...
    for (const strategy of searchStrategies) {
      ...
      try {
        ...
        if (results && results.length > 0) {
          ...
          console.log(`   📊 Employee IDs: ${results.join(', ')}`);
          return results[0]; // Return first ID
        } else {
  ...
  /**
   * Perform employee search to get IDs
   */
  async performEmployeeSearch(query) {
    ...
    if (data.hits?.hits?.length > 0) {
      return data.hits.hits.map(hit => hit._source.id);
    }
  ...
  /**
   * Step 2: Collect full employee data using ID
   */
  async collectEmployeeData(employeeId) {
    console.log(`\n📊 STEP 2: COLLECTING EMPLOYEE DATA FOR ID ${employeeId}...`);
    ...
    try {
      ...
      const data = await response.json();
      console.log(`   ✅ Collected data with ${Object.keys(data).length} fields`);
      
      return data;
    } catch (error) {
      console.error(`   ❌ Error collecting employee data: ${error.message}`);
      return null;
    }
  }

  /**
   * Step 3: Update person with complete employee data
   */
  async updatePersonWithEmployeeData(person, employeeData) {
    console.log('\n💾 STEP 3: UPDATING PERSON WITH EMPLOYEE DATA...');
      ...
      const updateData = {
        ...
        customFields: {
          ...
          coresignalEmployeeData: {
            ...
            enrichmentSource: 'CoreSignal Employee Search + Collect',
        ...
        enrichmentSources: [
          ...
          'coresignal-employee-search-collect'
  ...
  async enrichMultiplePeople(personNames) {
    ...
    for (const personName of personNames) {
      ...
      await this.enrichPersonWithSearchCollect(personName);
    }
...
// Export for use
module.exports = { EmployeeSearchCollectEnrichment };

// Example usage
async function enrichAaronAdkins() {
  const enrichment = new EmployeeSearchCollectEnrichment();
  await enrichment.enrichPersonWithSearchCollect('Aaron Adkins');
}
