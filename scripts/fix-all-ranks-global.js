...
/**
 * Fix All Ranks Global Script
 * 
 * This script assigns proper sequential ranks (1-N) to all records
 * in the database regardless of workspace, then we can filter by workspace in the UI.
 */
...
async function fixAllRanksGlobal() {
  try {
    console.log('🔧 Starting global ranks fix for all records...');
    ...
    // 5. Skip Opportunities (no rank field in schema)
    console.log('\n📊 Skipping Opportunities (no rank field in schema)...');
    ...
    console.log(`📊 Found ${opportunitiesCount} opportunities (no rank field to update)`);
    ...
    console.log('\n🎉 Global ranks fix completed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing global ranks:', error);
    throw error;
  } finally {
...
if (require.main === module) {
  fixAllRanksGlobal()
    .then(() => {
      console.log('🎉 Global ranks fix completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('💥 Global ranks fix failed:', error);
      process.exit(1);
    });
}

module.exports = { fixAllRanksGlobal };
