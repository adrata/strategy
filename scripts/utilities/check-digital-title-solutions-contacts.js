...
async function checkDigitalTitleSolutionsContacts() {
  try {
    await prisma.$connect();
    console.log('🔍 CHECKING DIGITAL TITLE SOLUTIONS CONTACTS\n');
    
    // Get the Digital Title Solutions account
    const digitalTitleAccount = await prisma.$queryRaw`
      SELECT 
        acc.id,
        acc.name,
        acc.phone,
    ...
    if (digitalTitleAccount.length > 0) {
      const account = digitalTitleAccount[0];
      ...
      if (contacts.length > 0) {
        console.log(`Found ${contacts.length} contacts at Digital Title Solutions:\n`);
        ...
        console.log('❌ No contacts found at Digital Title Solutions');
      }
    } else {
      console.log('❌ Digital Title Solutions account not found');
    }
...
checkDigitalTitleSolutionsContacts();
