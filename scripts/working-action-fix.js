...
async function main() {
  console.log('🔧 WORKING ACTION SYSTEM FIX');
  console.log('=============================\n');
  ...
  try {
    ...
    const orphanedActions = await prisma.actions.findMany({
      ...
      take: 300 // Process more in each batch
    });
    ...
    // Final status
    console.log('🎉 WORKING ACTION FIX COMPLETE!');
    console.log('================================');
    console.log(`✅ Fixed ${fixedCount} orphaned actions`);
    console.log(`✅ Updated lastAction for ${peopleUpdated} people`);
    console.log(`✅ Updated lastAction for ${companiesUpdated} companies`);
    ...
    console.error('❌ Error in working action fix:', error);
  } finally {
