...
/**
 * 🔍 ENRICH COMPANIES WITH PERPLEXITY (WITH ENV LOADING)
 * 
...
class EnrichCompaniesWithPerplexity {
  constructor() {
    ...
    console.log('PERPLEXITY_API_KEY:', this.perplexityApiKey ? 'SET' : 'NOT SET');
    console.log('CORESIGNAL_API_KEY:', this.coresignalApiKey ? 'SET' : 'NOT SET');
    
    if (!this.perplexityApiKey) {
      console.log('⚠️ PERPLEXITY_API_KEY not found in environment variables');
      console.log('   Please set PERPLEXITY_API_KEY in your .env file or environment');
    ...
    this.maxCompaniesPerRun = 3; // Very small batch for testing
  ...
  async execute() {
    console.log('🔍 ENRICHING COMPANIES WITH PERPLEXITY (ENV LOADING)');
    console.log('==================================================\n');
...
Please provide accurate, up-to-date information from reliable sources. Be concise and structured.`;
  ...
  async createPeopleAndBuyerGroup(company, employees) {
    ...
    // Collect first 5 employee profiles for testing
    const employeesToCollect = employees.slice(0, 5);
