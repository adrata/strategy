#!/usr/bin/env python3
"""
Corrected Adrata Branded Snyk Documents Generator
Word doc uses Inter font, PDF uses Helvetica (standard fonts) - no blue colors
"""
...
def create_word_document(desktop_path):
    filename = os.path.join(desktop_path, "Snyk_BGI_Transformation_Adrata_Final.docx")
    ...
    growth_para = doc.add_paragraph(
        "Snyk will continue growing—the question is velocity and scale. Analysis reveals two distinct "
        "trajectories based on stakeholder engagement sophistication:"
    )
...
def create_pdf_document(desktop_path):
    filename = os.path.join(desktop_path, "Snyk_BGI_Transformation_Adrata_Final.pdf")
    ...
    # Define styles with standard fonts (Helvetica family)
    ...
    # Custom styles with standard fonts
