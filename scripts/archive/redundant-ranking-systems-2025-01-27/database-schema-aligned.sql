-- =====================================================
-- RANKING EVENTS DATABASE SCHEMA - ALIGNED WITH EXISTING SCHEMA
-- =====================================================
-- This script creates the database schema for the
-- unified event-driven ranking system, aligned with existing models
-- =====================================================

-- Create ranking_events table (aligned with existing schema patterns)
...
-- Create indexes for ranking_events (aligned with existing patterns)
...
-- Create ranking_scores table for persistent storage (aligned with existing patterns)
...
-- Create indexes for ranking_scores (aligned with existing patterns)
...
-- Create ranking_metrics table for analytics (aligned with existing patterns)
...
-- Create indexes for ranking_metrics (aligned with existing patterns)
...
-- Create database triggers for automatic event generation (aligned with existing models)
...
BEGIN
    ...
    INSERT INTO ranking_events (
        ...
        COALESCE(NEW.assigned_user_id, OLD.assigned_user_id, 'system'),
        ...
        CASE 
            ...
            WHEN TG_TABLE_NAME = 'people' THEN 3
            WHEN TG_TABLE_NAME = 'companies' THEN 4
            ELSE 5
        END,
        NOW()
    );
...
-- Apply triggers to key tables (aligned with existing schema)
DROP TRIGGER IF EXISTS people_ranking_trigger ON people;
CREATE TRIGGER people_ranking_trigger
...
DROP TRIGGER IF EXISTS companies_ranking_trigger ON companies;
CREATE TRIGGER companies_ranking_trigger
