/**
 * Comprehensive Last Action & Next Action Audit Script
 * 
 * This script audits all people and company records to ensure:
...
async function auditLastNextActions() {
  console.log('🔍 LAST ACTION & NEXT ACTION AUDIT');
  console.log('====================================');
  ...
  try {
    ...
    const lastActionMismatches = peopleWithActions.filter(person => {
      ...
      return person.lastActionDate?.getTime() !== actualLastAction.completedAt?.getTime();
    });
    ...
    console.log('\n💡 RECOMMENDATION:');
    if (totalIssues > 0) {
      console.log('  Run `node scripts/populate-smart-actions.js` to fix all issues');
    } else {
...
// Run the audit
auditLastNextActions().catch(console.error);
