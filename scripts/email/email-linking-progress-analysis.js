...
async function emailLinkingProgressAnalysis() {
  try {
    ...
    // 3. LINKING RATE ANALYSIS
    console.log('🔗 LINKING RATE ANALYSIS:');
    console.log('-'.repeat(30));
    
    // Check recent linking activity
    const recentContactLinks = await prisma.emailToContact.count({
      where: {
        email_messages: {
          createdAt: {
            gte: oneDayAgo
    ...
    const recentAccountLinks = await prisma.emailToAccount.count({
      where: {
        email_messages: {
          createdAt: {
            gte: oneDayAgo
    ...
    const recentTotalLinks = recentContactLinks + recentAccountLinks + 
    ...
    console.log(`   Recent links (24h): ${recentTotalLinks}`);
    console.log(`   Linking rate: ${recentEmails > 0 ? (recentTotalLinks / recentEmails).toFixed(2) : '0.00'} links per email`);
    console.log('');
    ...
    targetCoverages.forEach(target => {
      if (target > currentCoverage) {
        ...
        const currentLinkingRate = recentTotalLinks / 24; // links per hour
        const hoursToComplete = currentLinkingRate > 0 ? neededLinks / currentLinkingRate : Infinity;
        const daysToComplete = hoursToComplete / 24;
