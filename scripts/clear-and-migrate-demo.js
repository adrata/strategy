...
/**
 * 🗑️🚀 CLEAR AND MIGRATE DEMO WORKSPACE
 * 
 * Clears existing Demo workspace data and migrates fresh data from old SBI database
 */
...
async function clearAndMigrateDemo() {
  try {
    console.log('🗑️🚀 Clearing and migrating Demo workspace...\n');
    ...
    // 1. Find Demo workspace in new database
    console.log('📋 FINDING DEMO WORKSPACE IN NEW DATABASE:');
    ...
    if (!newWorkspace) {
      throw new Error('Demo workspace not found in new database!');
    }
    
    console.log(`✅ Found workspace: ${newWorkspace.name} (${newWorkspace.id})\n`);
    ...
    // 3. Find Demo Workspace in SBI database (the one with 2000 companies, 19254 people)
    console.log('📋 FINDING DEMO WORKSPACE IN SBI DATABASE:');
    const sbiWorkspace = await sbiPrisma.$queryRaw`
      SELECT id, name, slug, timezone, description, "createdAt", "updatedAt"
      FROM workspaces 
      WHERE id = '01K1VBYX2YERMXBFJ60RC6J194'
      LIMIT 1;
    `;
    
    if (!sbiWorkspace || sbiWorkspace.length === 0) {
      throw new Error('Demo Workspace not found in SBI database!');
    }
    
    console.log(`✅ Found SBI workspace: ${sbiWorkspace[0].name} (${sbiWorkspace[0].id})\n`);

    // 4. Migrate ALL companies
    ...
    console.log('\n🎉 Demo workspace migration finished successfully!');
