...
/**
 * 🔍 VERIFY MULTIPLE PEOPLE IDENTITY
 * 
 * Test multiple people to verify we're getting the right person
 */

require('dotenv').config();

class VerifyMultiplePeopleIdentity {
  constructor() {
    this.apiKey = process.env.CORESIGNAL_API_KEY;
    this.baseUrl = 'https://api.coresignal.com/cdapi/v2';
  }

  async verifyMultiplePeople() {
    console.log('🔍 VERIFYING MULTIPLE PEOPLE IDENTITY');
    console.log('=====================================');
    console.log('Testing multiple people to verify we get the right person');
    console.log('');
    ...
    const testPeople = [
      {
        fullName: 'Adam Mattson',
        linkedinUrl: 'https://www.linkedin.com/in/adam-mattson-1a67ba208',
        ourCompany: 'Idaho Power Company'
      },
      {
        fullName: 'Aaron Adkins',
        linkedinUrl: 'https://www.linkedin.com/in/aaron-adkins-116b29170',
        ourCompany: 'Bartlett Electric Cooperative, Inc.'
      },
      {
        fullName: 'Kirt Mayson',
        linkedinUrl: 'https://www.linkedin.com/in/kirt-mayson-a9691580/',
        ourCompany: 'NorthWestern Energy'
      }
    ];
    
    for (const person of testPeople) {
      await this.verifySpecificPerson(person);
    }
  }
...
// Run multiple people identity verification
async function main() {
  const verifier = new VerifyMultiplePeopleIdentity();
  await verifier.verifyMultiplePeople();
}
...
module.exports = VerifyMultiplePeopleIdentity;
