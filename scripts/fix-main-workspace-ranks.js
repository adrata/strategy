...
/**
 * Fix Main Workspace Ranks Script
 * 
 * This script assigns proper sequential ranks (1-N) to all records
 * in the main workspace (01K5D01YCQJ9TJ7CT4DZDE79T1) which has:
 * - 4,218 people
 * - 2,384 leads  
 * - 587 prospects
 * - 516 companies
...
async function fixMainWorkspaceRanks() {
  try {
    console.log('🔧 Starting main workspace ranks fix...');
    
    // Define main workspace ID (the one with the most data)
    const mainWorkspaceId = '01K5D01YCQJ9TJ7CT4DZDE79T1';
    
    // 1. Fix Companies ranks (1-516)
    ...
    console.log(`✅ Final Results for workspace ${mainWorkspaceId}:`);
    console.log(`  Companies: ${finalCompanyCount}`);
    console.log(`  People: ${finalPeopleCount}`);
    console.log(`  Leads: ${finalLeadsCount}`);
    console.log(`  Prospects: ${finalProspectsCount}`);
    
    console.log('\n🎉 Main workspace ranks fix completed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing main workspace ranks:', error);
    throw error;
  } finally {
...
if (require.main === module) {
  fixMainWorkspaceRanks()
    .then(() => {
      console.log('🎉 Main workspace ranks fix completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('💥 Main workspace ranks fix failed:', error);
      process.exit(1);
    });
}

module.exports = { fixMainWorkspaceRanks };
