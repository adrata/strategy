...
/**
 * 🚀 SETUP ADRATA WORKSPACE AND ROSS SYLVESTER USER
 * 
...
class SetupAdrataWorkspaceAndRoss {
  constructor() {
    // Use the streamlined schema
  ...
  async setupWorkspaceAndUser() {
    ...
    console.log('=====================================================');
    console.log('Creating Adrata workspace and Ross Sylvester user account');
    console.log('');
    ...
    try {
      ...
      if (existingUser) {
        ...
        const needsUpdate = !existingUser.username;
        if (needsUpdate) {
      ...
      // Get the workspace and user IDs for linking
      ...
      if (!workspace || !user) {
        console.log('❌ ERROR: Could not find workspace or user for linking');
        return;
      }
...
// Run the setup
async function main() {
  const setup = new SetupAdrataWorkspaceAndRoss();
...
module.exports = SetupAdrataWorkspaceAndRoss;
