...
/**
 * 🚀 MIGRATE ADRATA WORKSPACE
 * 
 * Migrates Adrata workspace data from old SBI database to new streamlined database
 */
...
async function migrateAdrataWorkspace() {
  try {
    console.log('🚀 Migrating Adrata workspace data...\n');
    ...
    // 1. Find Adrata workspace in SBI database (the one with 433 companies, 42 people)
    console.log('📋 FINDING ADRATA WORKSPACE IN SBI DATABASE:');
    const sbiWorkspace = await sbiPrisma.$queryRaw`
      SELECT id, name, slug, timezone, description, "createdAt", "updatedAt"
      FROM workspaces 
      WHERE id = '01K1VBYXHD0J895XAN0HGFBKJP'
      LIMIT 1;
    `;
    
    if (!sbiWorkspace || sbiWorkspace.length === 0) {
      throw new Error('Adrata workspace not found in SBI database!');
    }
    
    console.log(`✅ Found SBI workspace: ${sbiWorkspace[0].name} (${sbiWorkspace[0].id})\n`);

    // 2. Find Adrata workspace in new database
    ...
    console.log('\n🎉 Adrata workspace migration finished successfully!');
