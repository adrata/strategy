...
async function main() {
  ...
  if (ross && ross.activeWorkspaceId) {
    ...
    // Test the v1 speedrun API to see current data
    console.log('\nTesting v1 speedrun API...');
    try {
      ...
      console.log('Speedrun API result:', result.success ? 'Success' : 'Failed');
      
      if (result.success && result.data) {
        console.log(`Found ${result.data.length} people in speedrun`);
      ...
      console.error('Error calling speedrun API:', error);
    }

    // Test the re-rank endpoint
    console.log('\nTesting v1 re-rank API...');
    try {
      ...
      if (result.success) {
        ...
        // Test the speedrun API again to see the updated data
        console.log('\nTesting speedrun API after re-ranking...');
        const response2 = await fetch(`http://localhost:3000/api/v1/speedrun?limit=50`, {
          method: 'GET',
          headers: {
            'x-workspace-id': ross.activeWorkspaceId,
            'x-user-id': ross.id
        ...
        console.log('Speedrun API after re-rank:', result2.success ? 'Success' : 'Failed');
        
        if (result2.success && result2.data) {
          console.log(`Found ${result2.data.length} people in speedrun after re-rank`);
          result2.data.slice(0, 5).forEach((person: any, index: number) => {
            console.log(`  ${index + 1}. Rank ${person.rank}: ${person.name} (${person.company?.name})`);
          });
        }
      }
