...
/**
 * Batch Person Enrichment Script
 * 
 * Enriches existing person records with data from CoreSignal, Perplexity, and Lusha
 * - Only enriches people with email OR LinkedIn URL
 * - Skips people already enriched in last 30 days
...
const options = {
  ...
  limit: null,
  skip: 0,
  generateIntelligence: !args.includes('--skip-intelligence')
};
...
class BatchPersonEnrichment {
   ...
   */
  async enrichPersonViaAPI(personId) {
  ...
  /**
   * Generate intelligence for person
   */
  ...
  needsEnrichment(person) {
    ...
    if (!person.email && !person.linkedinUrl) {
      return { needs: false, reason: 'No email or LinkedIn URL' };
    }

    // Check if recently enriched (skip unless --force)
    if (!this.options.force && person.lastEnriched) {
      const daysSince = (Date.now() - new Date(person.lastEnriched).getTime()) / (1000 * 60 * 60 * 24);
      if (daysSince < 30) {
    ...
    const missingFields = [];
    if (!person.jobTitle) missingFields.push('jobTitle');
    if (!person.department) missingFields.push('department');
    if (!person.state) missingFields.push('state');
    if (!person.bio) missingFields.push('bio');
  ...
  needsIntelligence(person) {
    ...
    if (!person.buyerGroupRole) missingIntelligence.push('buyerGroupRole');
    if (!customFields.influenceLevel) missingIntelligence.push('influenceLevel');
    if (!customFields.decisionPower) missingIntelligence.push('decisionPower');
    if (!customFields.engagementLevel) missingIntelligence.push('engagementLevel');

    if (missingIntelligence.length === 0) {
      return { needs: false, reason: 'Intelligence already generated' };
    }

    // Check if recently generated
    if (customFields.intelligenceGeneratedAt) {
      const daysSince = (Date.now() - new Date(customFields.intelligenceGeneratedAt).getTime()) / (1000 * 60 * 60 * 24);
      if (daysSince < 30) {
        return { needs: false, reason: `Intelligence generated ${Math.round(daysSince)} days ago` };
      }
    }

    return { needs: true, reason: `Missing: ${missingIntelligence.join(', ')}` };
  }
  ...
  async enrichPerson(person) {
    ...
    try {
      ...
      const check = this.needsEnrichment(person);
      
      if (!check.needs) {
        console.log(`   ⏭️  Skipped enrichment: ${check.reason}`);
        
        // Still check if intelligence needed
        if (this.options.generateIntelligence) {
          const intelCheck = this.needsIntelligence(person);
          if (intelCheck.needs) {
            console.log(`   🤖 Needs intelligence: ${intelCheck.reason}`);
            
            if (!this.options.dryRun) {
              const intelResult = await this.generateIntelligenceViaAPI(person.id);
              if (intelResult.success) {
                console.log(`   ✅ Intelligence generated`);
                this.stats.intelligenceGenerated++;
              }
            } else {
      ...
      if (this.options.dryRun) {
        console.log(`   🏃 DRY RUN: Would enrich this person`);
        this.stats.enriched++;
        return { success: true, dryRun: true };
      }

      // Call enrichment API
      const result = await this.enrichPersonViaAPI(person.id);
      ...
      if (result.status === 'completed') {
        ...
        // Generate intelligence after successful enrichment
        if (this.options.generateIntelligence) {
          await new Promise(resolve => setTimeout(resolve, 500)); // Brief pause
          
          try {
            const intelResult = await this.generateIntelligenceViaAPI(person.id);
            if (intelResult.success && !intelResult.data.cached) {
              console.log(`   🤖 Intelligence generated`);
              this.stats.intelligenceGenerated++;
            } else if (intelResult.success && intelResult.data.cached) {
              console.log(`   ℹ️  Using cached intelligence`);
            }
          } catch (intelError) {
            console.log(`   ⚠️  Intelligence generation failed: ${intelError.message}`);
          }
        }
  ...
  async run() {
    try {
      ...
      console.log(`Generate Intelligence: ${this.options.generateIntelligence ? 'YES' : 'NO'}`);
      ...
      console.log(`\n📊 Found ${people.length} people with email/LinkedIn\n`);

      if (people.length === 0) {
        console.log('No people to enrich. Exiting.');
        return;
      }
  ...
  printProgress() {
    ...
    console.log(`   🤖 Intelligence: ${this.stats.intelligenceGenerated}`);
  ...
  printFinalReport() {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📊 BATCH PERSON ENRICHMENT COMPLETE');
    console.log('='.repeat(60));
    console.log(`Total Processed: ${this.stats.totalProcessed}`);
    console.log(`✅ Enriched: ${this.stats.enriched}`);
    console.log(`🤖 Intelligence Generated: ${this.stats.intelligenceGenerated}`);
    console.log(`⏭️  Skipped: ${this.stats.skipped}`);
    console.log(`❌ Failed: ${this.stats.failed}`);
...
Usage:
  node scripts/batch-enrich-people.js [options]
...
Options:
  ...
  --skip-intelligence     Skip intelligence generation (enrichment only)
...
`);
  ...
  const enrichment = new BatchPersonEnrichment(options);
