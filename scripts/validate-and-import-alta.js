...
async function main() {
  try {
    console.log('📁 Loading ALTA contacts from file...');
    
    // Try different approaches to read the file
    let rawData;
    try {
      rawData = fs.readFileSync('alta-contacts.json', 'utf8');
    } catch (error) {
      console.log('❌ Failed to read file with utf8, trying with different encoding...');
      rawData = fs.readFileSync('alta-contacts.json', 'utf16le');
    }
    
    console.log(`📄 File size: ${rawData.length} characters`);
    
    // Clean up the data - remove any BOM or extra characters
    rawData = rawData.replace(/^\uFEFF/, '').trim();
    
    console.log('🔍 Parsing JSON...');
    const contactsData = JSON.parse(rawData);
    
    console.log(`✅ Loaded ${contactsData.length} contacts from file`);
    
    // Run the import
    const stats = await importAltaContacts(contactsData);
