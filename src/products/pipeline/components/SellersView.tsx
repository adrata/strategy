    ...
    return (
      <div className="p-6">
        <div className="animate-pulse">
          <div className="h-8 bg-loading-bg rounded w-1/4 mb-6"></div>
          <div className="space-y-4">
            {[1, 2].map((i) => (
              <div key={i} className="h-20 bg-loading-bg rounded"></div>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-background border border-border rounded-lg p-8">
            <div className="flex items-center mb-6">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">{selectedPerson.fullName}</h1>
                <p className="text-xl text-muted">{selectedPerson.jobTitle}</p>
                <p className="text-lg text-muted">{selectedPerson.company}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-foreground">{selectedPerson.email}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-muted">Phone:</span>
                    <p className="text-foreground">{selectedPerson.phone}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-muted">Location:</span>
                    <p className="text-foreground">{selectedPerson.city}, {selectedPerson.state}</p>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <span className="text-sm font-medium text-muted">Wants:</span>
                    <p className="text-foreground">Increase conversion rates, optimize user experience, reduce customer acquisition costs</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-muted">Needs:</span>
                    <p className="text-foreground">Better analytics tools, A/B testing platform, customer journey mapping</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-muted">Pain Points:</span>
                    <p className="text-foreground">Current tools are fragmented, lack of real-time insights, manual reporting processes</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-muted">Goals:</span>
                    <p className="text-foreground">Improve conversion rates by 25%, reduce CAC by 30%, increase customer lifetime value</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-border">
              <h3 className="text-lg font-semibold text-foreground mb-4">Reports & Analysis</h3>
  ...
  if (selectedCompany) {
    ...
    return (
      <div className="p-6">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-foreground mb-2">{selectedCompany} Buyer Group</h1>
          <p className="text-muted">Key stakeholders and decision makers</p>
        </div>
          ...
          {buyerGroup.map((person) => (
            ...
            >
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted">Flight Risk:</span>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted">Influence:</span>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
      ...
      <div className="p-6">
          ...
          <div className="flex items-center mb-6">
            ...
            <div>
              <div className="flex items-center mb-1">
                <h1 className="text-3xl font-bold text-foreground">{selectedSeller.name}</h1>
                {isCurrentUserLeader && (
              ...
              <p className="text-muted text-lg">Senior Account Executive - Enterprise West</p>
              <div className="flex items-center mt-2">
                <div className={`w-3 h-3 rounded-full mr-2 ${selectedSeller.isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                <span className="text-sm text-muted">{selectedSeller.isOnline ? 'Online' : 'Offline'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-background border border-border rounded-lg p-6">
            ...
            <div className="text-sm text-muted">Decision Maker Engagement</div>
          </div>
          <div className="bg-background border border-border rounded-lg p-6">
            <div className="text-2xl font-bold text-green-600 mb-1">4.5</div>
            <div className="text-sm text-muted">Avg Stakeholders/Group</div>
          </div>
          <div className="bg-background border border-border rounded-lg p-6">
            <div className="text-2xl font-bold text-purple-600 mb-1">40/50</div>
            <div className="text-sm text-muted">Active Buyer Groups</div>
          </div>
          <div className="bg-background border border-border rounded-lg p-6">
            <div className="text-2xl font-bold text-orange-600 mb-1">105%</div>
            <div className="text-sm text-muted">% to Goal</div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-background border border-border rounded-lg p-6 animate-pulse">
                <div className="h-6 bg-loading-bg rounded w-3/4 mb-2"></div>
                <div className="h-4 bg-loading-bg rounded w-1/2 mb-4"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sellerCompanies.length > 0 ? (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-muted">{company.industry || 'Unknown'} • {company.size || 'Unknown'}</p>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-muted">Revenue</div>
                      <div className="text-lg font-semibold text-foreground">$100M+</div>
                    </div>
                  </div>
              
                  <div className="space-y-3 mb-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted">Buyer Group Identified</span>
                      <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                        Complete
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted">Deal Size</span>
                      <span className="text-sm font-medium text-foreground">$250K</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted">Buyer Group Engaged</span>
              ...
              <div className="col-span-full text-center py-12">
                <div className="text-muted text-lg">No companies assigned to this seller</div>
                <div className="text-muted text-sm mt-2">Companies will appear here once they are assigned</div>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="text-muted text-lg">Sales Team</p>
      </div>
