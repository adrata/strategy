...
interface AiPersonFinderProps {
  isMinimized?: boolean;
  onMinimize?: () => void;
  onExpand?: () => void;
}

export function AiPersonFinder({
  isMinimized = false,
  onMinimize,
  onExpand,
}: AiPersonFinderProps) {
  ...
  if (isMinimized) {
    return (
      <button
        onClick={onExpand}
        className="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-full text-sm font-medium hover:from-purple-600 hover:to-blue-600 transition-all shadow-md"
      >
        <MagnifyingGlassIcon className="w-4 h-4" />
        <span>AI Person Finder</span>
      </button>
    );
  }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const monacoResponse = await fetch("/api/enrichment", {
        method: "POST",
      ...
      if (monacoResponse.ok) {
        const monacoData = await monacoResponse.json();
          ...
          setTimeout(async () => {
            try {
              ...
              const enrichmentMessage = {
                ...
                type: "assistant" as const,
                content: `🧠 Monaco intelligence pipeline started for ${person.name}. Enriching their profile and buyer group context...`,
                timestamp: new Date(),
              };
              ...
              console.warn("Failed to trigger enrichment for new lead");
            }
          }, 1000);
        }
      } else {
