...
export const Loader: React.FC<LoaderProps> = ({
  ...
  const containerClasses = fullScreen 
    ? `fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center ${className}`
    : `flex flex-col items-center justify-center p-8 ${className}`;

  return (
    <div className={containerClasses}>
      {/* Beautiful spinning loader */}
      <div className={`${sizeClasses[size]} border-2 border-border border-t-blue-500 rounded-full animate-spin mb-4`} />
      
      {/* Loading message */}
      <p className={`text-muted ${textSizeClasses[size]} animate-pulse`}>
        {message}
      </p>
    </div>
  );
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className="h-4 bg-loading-bg rounded animate-pulse"
          style={{
...
export const PanelLoader: React.FC<{ message?: string }> = ({ message }) => (
  <div className="flex items-center justify-center p-8">
    <div className="text-center">
      <div className="h-8 w-8 bg-loading-bg rounded animate-pulse mx-auto mb-2"></div>
    ...
    <div className="bg-background border-b border-border px-6 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-8">
          <div>
            <div className="h-8 w-32 bg-loading-bg rounded animate-pulse mb-2"></div>
            <div className="h-4 w-24 bg-loading-bg rounded animate-pulse"></div>
          </div>
          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="h-6 w-16 bg-loading-bg rounded animate-pulse mb-1"></div>
              <div className="h-3 w-12 bg-loading-bg rounded animate-pulse"></div>
            </div>
            <div className="text-center">
              <div className="h-6 w-12 bg-loading-bg rounded animate-pulse mb-1"></div>
              <div className="h-3 w-8 bg-loading-bg rounded animate-pulse"></div>
            </div>
          </div>
        </div>
        <div className="h-10 w-24 bg-loading-bg rounded animate-pulse"></div>
      </div>
    </div>
    ...
    <div className="flex-shrink-0 px-6 pt-2 pb-1">
      <div className="flex items-center justify-between gap-4 py-2">
        <div className="flex items-center gap-4">
          <div className="h-10 w-64 bg-loading-bg rounded animate-pulse"></div>
          <div className="h-10 w-32 bg-loading-bg rounded animate-pulse"></div>
        </div>
        ...
        <div className="p-4 border-b border-border">
          <div className="grid grid-cols-5 gap-4">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-4 bg-loading-bg rounded animate-pulse"></div>
            ))}
          </div>
        </div>
          ...
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="grid grid-cols-5 gap-4">
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="h-4 bg-loading-bg rounded animate-pulse"></div>
              ))}
            </div>
          ))}
        </div>
...
export const CompanyDetailSkeleton: React.FC<{ message?: string }> = ({ message }) => (
  <div className="h-full flex flex-col bg-background">
    ...
    <div className="flex-shrink-0 px-6 pt-2 pb-1">
      <div className="flex items-center gap-8">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-10 w-20 bg-loading-bg rounded animate-pulse"></div>
        ))}
      </div>
    </div>
    ...
    <div className="flex-1 px-6 py-6 min-h-0">
      <div className="space-y-8">
        ...
        {/* Additional Section */}
        <div className="space-y-4">
          <div className="h-6 w-36 bg-loading-bg rounded animate-pulse"></div>
