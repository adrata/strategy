...
export function AddFilesPopup({ isOpen, onClose, onFileSelect, onAddFiles }: AddFilesPopupProps) {
  ...
  return (
    <>
      ...
      <div className="absolute left-0 bottom-12 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl py-2 min-w-[320px] max-w-[400px] z-50">
        ...
        {showDataOptions && (
          <div className="px-4 pb-3">
            <div className="relative">
              <MagnifyingGlassIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 w-3 h-3 text-gray-400" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={`Search ${DATA_CATEGORIES.find(cat => cat['id'] === activeDataCategory)?.label.toLowerCase() || 'data'}...`}
                className="w-full pl-8 pr-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-red-500 focus:border-red-500"
              />
            </div>
          </div>
        )}
          ...
          {DATA_CATEGORIES.map((category) => {
            ...
            return (
              <button
                ...
                className={`w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center space-x-3 transition-colors ${
                  isActive && showDataOptions ? 'bg-gray-50 dark:bg-gray-700' : ''
                }`}
              >
                <IconComponent className="w-4 h-4 text-gray-500" />
                <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {selectedItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="inline-flex items-center space-x-1 px-2 py-1 rounded text-xs bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                    >
              ...
              {recentFiles.slice(0, 3).map((file) => {
                ...
                return (
                  <button
                    ...
                    className="w-full px-4 py-2 text-left text-sm text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center space-x-3 transition-colors"
                  >
