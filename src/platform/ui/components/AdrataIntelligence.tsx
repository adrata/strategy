  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <button
        ...
        className="group flex items-center space-x-3 px-4 py-3 bg-background/50 backdrop-blur-sm border border-white/20 rounded-xl hover:bg-background/70 transition-all duration-300 shadow-lg"
      >
        <div
        ...
        <div className="flex-1 text-left">
          <div className="flex items-center space-x-2">
            <span className="font-semibold text-foreground">Adrata</span>
            <span className={`text-sm font-medium ${currentModeData.color}`}>
              {currentModeData.name}
            </span>
          </div>
          <p className="text-xs text-muted">{currentModeData.description}</p>
        </div>
      ...
      {isExpanded && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-background/95 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl z-50 overflow-hidden">
            <div className="p-6">
              <div className="mb-6">
                    ...
                    <button
                      ...
                      className={`w-full flex items-start space-x-4 p-4 rounded-xl transition-all duration-200 text-left group ${
                        isActive
                          ...
                          : "hover:bg-panel-background border border-transparent"
                      }`}
                    >
                      <div
                        ...
                        <div className="space-y-1">
                          {mode.examples
                            ...
                            .map((example, idx) => (
                              <div
                                key={idx}
                                className="flex items-center space-x-2 text-xs text-muted"
                              >
