  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm">
      <div className="flex items-start justify-center pt-[15vh] px-4">
        <div className="w-full max-w-4xl bg-background dark:bg-foreground rounded-xl shadow-2xl border border-border dark:border-border overflow-hidden">
          {/* Search Header */}
          <div className="border-b border-border dark:border-border">
            ...
            <div className="flex items-center px-4 py-3">
              <input
                ...
                className="flex-1 bg-transparent outline-none text-foreground placeholder-[var(--muted)] text-lg ml-3"
                autoFocus
              />
              {isLoading && (
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                      selectedFilter === filter.id
                        ...
                        : "text-muted dark:text-muted hover:bg-hover"
                    }`}
                  >
            ...
            {!query.trim() ? (
              ...
              <div className="p-4">
                {recentSearches.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-sm font-medium text-foreground dark:text-foreground mb-2 flex items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {recentSearches.slice(0, 5).map((search, index) => (
                        <button
                          ...
                          className="px-3 py-1.5 text-sm bg-hover text-gray-700 dark:text-gray-300 rounded-lg hover:bg-loading-bg transition-colors"
                        >
                          {search}
                        </button>
                      ))}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className="p-3 text-left bg-panel-background rounded-lg hover:bg-hover transition-colors"
                      >
                        <div className="font-medium text-foreground dark:text-foreground text-sm">
                          {item.title}
                        </div>
                ...
                {filteredResults.map((result, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-start px-4 py-3 text-left hover:bg-panel-background transition-colors ${
                        index === selectedIndex
                          ? "bg-blue-50 dark:bg-blue-900/20 border-r-2 border-blue-500"
                          : ""
                      }`}
                    >
                      <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-hover mr-3 mt-0.5">
                        <TypeIcon className="w-5 h-5 text-muted dark:text-muted" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="px-2 py-0.5 text-xs bg-hover text-muted dark:text-muted rounded-full">
                            {result.source}
                          </span>
                        </div>
                        <p className="text-sm text-muted dark:text-muted mb-1">
                          {result.description}
                        </p>
                        {result['preview'] && (
                              ...
                              <div className="flex gap-1">
                                {result.metadata.tags
                                  ...
                                  .map((tag, i) => (
                                    <span
                                      key={i}
                                      className="px-1.5 py-0.5 bg-hover rounded text-xs"
                                    >
                                      {tag}
                                    </span>
                                  ))}
