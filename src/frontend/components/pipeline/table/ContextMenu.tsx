...
export function ContextMenu({ x, y, record, section, onClose, onDelete }: ContextMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      style={menuStyle}
      className="bg-panel-background border border-border rounded-md shadow-lg py-1 min-w-[120px]"
    >
      <button
        onClick={handleView}
        className="w-full px-3 py-2 text-left text-sm text-foreground hover:bg-hover transition-colors flex items-center gap-2"
      >
      ...
      <button
        ...
        className="w-full px-3 py-2 text-left text-sm text-error hover:bg-error/10 transition-colors flex items-center gap-2"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
