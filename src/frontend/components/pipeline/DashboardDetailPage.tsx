...
export function DashboardDetailPage({ statType }: DashboardDetailPageProps) {
  ...
  const renderActivityItem = (activity: Activity | DealActivity, index: number) => {
    ...
    if (statType === 'deals' || statType === 'total-pipeline' || statType === 'deals-closed' || statType === 'avg-deal-size' || statType === 'pipeline-value-added') {
      ...
      return (
        <div key={deal.id} className={`group p-6 rounded-xl border ${config.borderColor} bg-white hover:shadow-md transition-all duration-200 hover:scale-[1.02]`}>
      ...
      return (
        <div key={act.id} className={`group p-6 rounded-xl border ${config.borderColor} bg-white hover:shadow-md transition-all duration-200 hover:scale-[1.02]`}>
          <div className="flex items-start space-x-4">
            <div className={`flex-shrink-0 w-12 h-12 ${config.bgColor} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}>
              <Icon className={`w-6 h-6 ${config.iconColor}`} />
            </div>
            ...
            {loading ? (
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 mt-4">Loading {config.title.toLowerCase()}...</p>
                </div>
              </div>
            ) : error ? (
              <div className="flex items-center justify-center h-64">
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center max-w-md">
                  ...
                  <p className="text-gray-600">{config.emptyDescription}</p>
                </div>
              </div>
            ) : (
