...
export function SimpleBulletproofPipelineView({ section }: SimpleBulletproofPipelineViewProps) {
    ...
    return (
      <div className="h-full bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-muted">{error instanceof Error ? error.message : String(error)}</p>
        </div>
      </div>
    );
  }
  ...
  if (data['length'] === 0) {
    return (
      <div className="h-full bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="text-muted text-2xl mb-4">📋</div>
          <h4 className="text-lg font-medium text-foreground mb-2">No {section} yet</h4>
          <p className="text-sm text-muted">Add your first {section.slice(0, -1)} to get started</p>
        </div>
      </div>
    );
  }
