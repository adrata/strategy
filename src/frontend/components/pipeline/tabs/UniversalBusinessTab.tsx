...
export function UniversalBusinessTab({ record, recordType, onSave }: UniversalBusinessTabProps) {
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* Business Challenges */}
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-4">Business Challenges & Priorities</h3>
      ...
      {/* Industry Context */}
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-4">Industry Context</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-panel-background rounded-lg p-4">
            <h4 className="font-medium text-foreground mb-3">Company Profile</h4>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-foreground">{record?.marketPosition || 'Established'}</span>
              </div>
            </div>
          </div>

          <div className="bg-panel-background rounded-lg p-4">
            <h4 className="font-medium text-foreground mb-3">Decision Making</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted">Budget Cycle:</span>
                <span className="font-medium text-foreground">{record?.budgetCycle || 'Annual'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted">Approval Process:</span>
                <span className="font-medium text-foreground">{record?.approvalProcess || 'Committee'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted">Decision Timeline:</span>
                <span className="font-medium text-foreground">{record?.decisionTimeline || '3-6 months'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted">Key Influencers:</span>
                <span className="font-medium text-foreground">{record?.keyInfluencers || 'C-Level'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-4">Strategic Initiatives</h3>
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4">
            <h4 className="font-medium text-foreground mb-2">Current Initiatives</h4>
            <div className="space-y-2 text-sm text-gray-700">
              {record?.strategicInitiatives?.length > 0 ? (
                ...
                <div>
                  ...
                  <p className="text-muted italic mt-2">Ask about current strategic projects</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2 text-sm text-gray-700">
              {record?.successMetrics?.length > 0 ? (
                ...
                <div>
                  ...
                  <p className="text-muted italic mt-2">Understand how they measure success</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-yellow-50 rounded-lg p-4">
            <h4 className="font-medium text-foreground mb-2">Market Threats</h4>
