...
export function ProspectOverviewTab({ recordType, record: recordProp, onSave }: ProspectOverviewTabProps) {
  ...
  useEffect(() => {
    const triggerEnrichment = async () => {
      ...
      if (hasBeenEnriched) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`ℹ️ [PROSPECT OVERVIEW] Record ${record.id} already enriched, skipping auto-enrichment`);
        }
        return;
      }
      ...
      if (hasIdentifier && missingCriticalData) {
        console.log(`🤖 [PROSPECT OVERVIEW] Auto-triggering enrichment for unenriched person: ${record.id}`);
        ...
        try {
          ...
          if (enrichResult?.status === 'completed') {
            ...
            if (typeof window !== 'undefined' && record?.id) {
              ...
              console.log(`🔄 [PROSPECT OVERVIEW] Cache invalidated, record will refresh on next load`);
            }
