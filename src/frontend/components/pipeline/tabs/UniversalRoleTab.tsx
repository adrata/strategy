...
export function UniversalRoleTab({ recordType, record: recordProp, onSave }: UniversalRoleTabProps) {
  ...
  const roleData = {
    ...
    influenceLevel: record?.influenceLevel || record?.customFields?.influenceLevel || record?.customFields?.enrichedData?.overview?.influenceLevel || 'Medium',
    engagementPriority: record?.priority || record?.customFields?.priority || record?.customFields?.enrichedData?.overview?.priority || 'MEDIUM',
