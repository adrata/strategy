...
export function UniversalHistoryTab({ recordType, record: recordProp }: UniversalHistoryTabProps) {
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div>
        <h3 className="text-lg font-semibold text-[var(--foreground)] mb-4">Interaction Timeline</h3>
        <div className="space-y-4">
          {historyData.map((activity: any, index: number) => (
            <div key={activity.id || index} className="bg-[var(--background)] p-4 rounded-lg border border-[var(--border)]">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0">
                  <div className="w-10 h-10 bg-[var(--hover)] rounded-full flex items-center justify-center">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-semibold text-[var(--foreground)]">{activity.subject || 'Activity'}</h4>
                      <p className="text-sm text-[var(--muted)]">{activity.type || 'Unknown'}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    {activity.duration && (
                      <span className="text-sm text-[var(--muted)]">Duration: {activity.duration}</span>
                    )}
                  </div>
                  ...
                  {activity.outcome && (
                    <div className="mb-2">
                      ...
                      <span className="text-sm text-[var(--muted)]">{activity.outcome}</span>
                    </div>
                  )}
                  ...
                  {activity.participants && activity.participants.length > 0 && (
                    <div className="mb-2">
                      ...
                      <span className="text-sm text-[var(--muted)]">{activity.participants.join(', ')}</span>
                    </div>
                  )}
                  ...
                  {activity.notes && (
                    <div>
                      ...
                      <span className="text-sm text-[var(--muted)]">{activity.notes}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[var(--background)] p-4 rounded-lg border border-[var(--border)]">
            <h4 className="font-medium text-[var(--foreground)] mb-2">Total Interactions</h4>
          ...
          <div className="bg-[var(--background)] p-4 rounded-lg border border-[var(--border)]">
            <h4 className="font-medium text-[var(--foreground)] mb-2">Last Activity</h4>
            <div className="text-sm text-[var(--muted)]">
          ...
          <div className="bg-[var(--background)] p-4 rounded-lg border border-[var(--border)]">
            <h4 className="font-medium text-[var(--foreground)] mb-2">Activity Types</h4>
            <div className="text-sm text-[var(--muted)]">
