...
export function ReleaseNotesModal({ isOpen, onClose, onSave, shippedItems }: ReleaseNotesModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[var(--background)] rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col mx-4">
        {/* Header */}
          ...
          <div>
            ...
            <div className="max-h-48 overflow-y-auto border border-[var(--border)] rounded-lg p-3 space-y-2">
              {shippedItems.map((item) => (
                <label
                  key={item.id}
                  className="flex items-center gap-3 p-2 hover:bg-[var(--hover)] rounded cursor-pointer"
                >
                  <input
                    type="checkbox"
