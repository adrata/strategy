      ...
      <div className="h-full flex flex-col bg-[var(--background)]">
        ...
        <div className="flex-shrink-0 px-6 py-4 border-b border-[var(--border)]">
          <div className="flex items-center justify-between">
            <div>
              <div className="h-8 w-40 bg-[var(--loading-bg)] rounded animate-pulse mb-2"></div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="bg-[var(--panel-background)] border border-[var(--border)] rounded-lg p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <div className="h-6 w-24 bg-[var(--loading-bg)] rounded animate-pulse"></div>
                ...
                <div className="mt-3 pt-3 border-t border-[var(--border)]">
                  <div className="h-3 w-20 bg-[var(--loading-bg)] rounded animate-pulse mb-2"></div>
                  <div className="flex gap-1">
                    <div className="h-5 w-12 bg-[var(--loading-bg)] rounded animate-pulse"></div>
                    <div className="h-5 w-16 bg-[var(--loading-bg)] rounded animate-pulse"></div>
                    <div className="h-5 w-14 bg-[var(--loading-bg)] rounded animate-pulse"></div>
                  </div>
                </div>
              </div>
            ))}
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--background)]">
      ...
      <div className="flex-shrink-0 px-6 py-4 border-b border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-[var(--foreground)]">Database Models</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {models.map((model) => (
            <div
              ...
              className="bg-[var(--panel-background)] border border-[var(--border)] rounded-lg p-4 hover:bg-[var(--hover)] cursor-pointer transition-colors shadow-sm"
            >
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[var(--muted)]">Fields:</span>
                  <span className="text-sm font-medium text-[var(--foreground)]">{model.fields.length}</span>
                </div>
              ...
              <div className="mt-3 pt-3 border-t border-[var(--border)]">
                <div className="text-xs text-[var(--muted)] mb-2">Key Fields:</div>
                <div className="flex gap-1 flex-wrap">
                  {model.fields.slice(0, 3).map((field) => (
                    <span key={field.name} className="text-xs bg-[var(--background)] text-[var(--muted)] px-2 py-1 rounded">
                      {field.name}
                    </span>
                  ))}
                  {model.fields.length > 3 && (
                    <span className="text-xs bg-[var(--background)] text-[var(--muted)] px-2 py-1 rounded">
                      +{model.fields.length - 3}
                    </span>
                  )}
                </div>
