...
export function StreamlinedAttributesView() {
  ...
  const handleFieldClick = (modelName: string, fieldName: string) => {
    router.push(`/adrata/database/objects/${modelName}/fields/${fieldName}`);
  };
                ...
                <div className="p-4">
                  <div className="space-y-2">
                    {Array.from({ length: 4 }).map((_, fieldIndex) => (
                      <div key={fieldIndex} className="flex items-center justify-between p-2 border border-[var(--border)] rounded">
                        <div className="flex items-center gap-2">
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--background)]">
      ...
      <div className="flex-shrink-0 px-6 py-4 border-b border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-[var(--foreground)]">Attributes</h1>
            <p className="text-sm text-[var(--muted)] mt-1">
              Object fields & properties ({models.reduce((sum, model) => sum + model.fields.length, 0)} attributes)
            </p>
          </div>
        </div>
      </div>
          ...
          {models.map((model) => (
            <div key={model.name} className="bg-white border border-[var(--border)] rounded-lg p-4 hover:bg-gray-50 cursor-pointer transition-colors shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[var(--muted)]">Primary Keys:</span>
                  <span className="text-sm font-medium text-[var(--foreground)]">
                    {model.fields.filter(f => f.isPrimaryKey).length}
                  </span>
                </div>
              </div>
