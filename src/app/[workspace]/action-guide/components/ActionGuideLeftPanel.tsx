...
export function ActionGuideLeftPanel() {
  ...
  const handlePageClick = (page: ActionGuidePage) => {
  ...
  const totalDocuments = actionGuideContent.sections.reduce((total, section) => total + section.pages.length, 0);
  const gettingStartedCount = actionGuideContent.sections.find(s => s.id === 'getting-started')?.pages.length || 0;
  const coreFeaturesCount = actionGuideContent.sections.find(s => s.id === 'core-features')?.pages.length || 0;
  const commonTasksCount = actionGuideContent.sections.find(s => s.id === 'common-tasks')?.pages.length || 0;
  const advancedFeaturesCount = actionGuideContent.sections.find(s => s.id === 'advanced-features')?.pages.length || 0;

  const sections = [
    {
      id: "getting-started",
      name: "Getting Started",
      description: "Welcome and basics",
  ...
  return (
    <div className="w-[13.085rem] min-w-[13.085rem] max-w-[13.085rem] bg-[var(--background)] text-[var(--foreground)] border-r border-[var(--border)] flex flex-col h-full">
        ...
        <div className="mx-2 mt-4 mb-2">
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-[var(--background)] border border-[var(--border)] overflow-hidden" style={{ filter: 'none' }}>
              <InformationCircleIcon className="w-5 h-5 text-[var(--foreground)]" />
            </div>
            <div className="flex-1">
              <div className="flex items-center mb-1">
                ...
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--hover)] text-[var(--foreground)]">
                  Help
                </span>
              </div>
        ...
        <div className="mx-2 mb-4 p-3 bg-[var(--hover)] rounded-lg border border-[var(--border)]">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-xs font-medium text-[var(--muted)]">Guides</span>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-semibold text-[var(--foreground)]">
                {actionGuideContent.sections.length}
              </span>
            </div>
            <div className="flex justify-between items-center">
      ...
      {/* Scrollable Middle Section - Guide Sections */}
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${
                    isActive
                      ...
                      : 'hover:bg-[var(--panel-background)] text-[var(--muted)]'
                  }`}
                >
