    ...
    <div className="h-full flex flex-col bg-[var(--background)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-[var(--foreground)]">{connection.connectionName || connection.provider}</h2>
            <p className="text-sm text-[var(--muted)]">{connection.provider}</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto invisible-scrollbar">
        ...
        {/* Connection Info */}
        <div className="space-y-3">
          <h3 className="font-medium text-[var(--foreground)]">Connection Details</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-[var(--muted)]">Provider:</span>
              <span className="text-[var(--foreground)] font-medium">{connection.provider}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--muted)]">Connection ID:</span>
              <span className="text-[var(--foreground)] font-mono text-xs">{connection.nangoConnectionId}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--muted)]">Created:</span>
              <span className="text-[var(--foreground)]">{formatDate(connection.createdAt)}</span>
            </div>
            {connection.lastSyncAt && (
              <div className="flex justify-between">
                <span className="text-[var(--muted)]">Last Sync:</span>
                <span className="text-[var(--foreground)]">{formatDate(connection.lastSyncAt)}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {connection.metadata && Object.keys(connection.metadata).length > 0 && (
          <div className="space-y-3">
            <h3 className="font-medium text-[var(--foreground)]">Additional Information</h3>
        ...
        {/* Actions */}
        <div className="space-y-3">
          <h3 className="font-medium text-[var(--foreground)]">Actions</h3>
          <div className="space-y-2">
            <button className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-700 bg-[var(--hover)] rounded-lg hover:bg-[var(--loading-bg)] transition-colors">
        ...
        <div className="space-y-3 pt-4 border-t border-[var(--border)]">
          <h3 className="font-medium text-[var(--foreground)]">Danger Zone</h3>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 px-3 py-2 text-sm font-medium text-gray-700 bg-[var(--hover)] rounded-lg hover:bg-[var(--loading-bg)] transition-colors"
                >
                  Cancel
                </button>
              </div>
