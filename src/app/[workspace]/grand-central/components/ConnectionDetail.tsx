    ...
    <div className="h-full flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-foreground">{connection.connectionName || connection.provider}</h2>
            <p className="text-sm text-muted">{connection.provider}</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto invisible-scrollbar">
        ...
        {/* Connection Info */}
        <div className="space-y-3">
          <h3 className="font-medium text-foreground">Connection Details</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-muted">Provider:</span>
              <span className="text-foreground font-medium">{connection.provider}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Connection ID:</span>
              <span className="text-foreground font-mono text-xs">{connection.nangoConnectionId}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Created:</span>
              <span className="text-foreground">{formatDate(connection.createdAt)}</span>
            </div>
            {connection.lastSyncAt && (
              <div className="flex justify-between">
                <span className="text-muted">Last Sync:</span>
                <span className="text-foreground">{formatDate(connection.lastSyncAt)}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <button className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-700 bg-hover rounded-lg hover:bg-loading-bg transition-colors">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 px-3 py-2 text-sm font-medium text-gray-700 bg-hover rounded-lg hover:bg-loading-bg transition-colors"
                >
                  Cancel
                </button>
              </div>
