...
export function FolderTree({ onFolderSelect }: FolderTreeProps) {
  ...
  const loadFolders = async () => {
    try {
      ...
      // Mock data for now
      const mockFolders: WorkbenchFolder[] = [
        {
          id: '1',
          name: 'Projects',
  ...
  const buildFolderTree = (parentId: string | null = null, level: number = 0): WorkbenchFolder[] => {
    return folders
      .filter(folder => folder.parentId === parentId)
      .sort((a, b) => a.name.localeCompare(b.name));
  };

  const renderFolder = (folder: WorkbenchFolder, level: number = 0) => {
