...
export function CreateExperimentModal({ onClose }: CreateExperimentModalProps) {
    ...
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-2">Experiment Name</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-2">Hypothesis</label>
              <textarea
      ...
      case 2:
        return (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-2">Experiment Type</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-2">Target Sample Size</label>
              <input
                type="number"
        ...
        return (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-[var(--foreground)]">Variants</h3>
              <button
                type="button"
                onClick={addVariant}
            ...
            {formData.variants.map((variant, index) => (
              <div key={index} className="p-4 border border-[var(--border)] rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-medium text-[var(--foreground)]">Variant {index + 1}</h4>
                  ...
                  <div className="flex items-center space-x-4">
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm text-[var(--foreground)]">Control Group</span>
                    </label>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-[var(--foreground)] capitalize">{formData.experimentType.replace('_', ' ')}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted-foreground)]">Sample Size:</span>
                <span className="text-[var(--foreground)]">{formData.targetSampleSize}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted-foreground)]">Variants:</span>
                <span className="text-[var(--foreground)]">{formData.variants.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--muted-foreground)]">Confidence Level:</span>
                <span className="text-[var(--foreground)]">{(formData.confidenceLevel * 100).toFixed(0)}%</span>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[var(--background)] rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-[var(--border)]">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-[var(--foreground)]">Create Experiment</h2>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 border-t border-[var(--border)] flex justify-between">
            <button
              ...
              className="px-4 py-2 border border-[var(--border)] text-[var(--foreground)] rounded-lg hover:bg-[var(--hover)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Previous
            </button>
