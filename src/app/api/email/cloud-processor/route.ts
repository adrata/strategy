...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔗 Loaded entities: ${people.length} people, ${companies.length} companies, ${leads.length} leads, ${opportunities.length} opportunities, ${prospects.length} prospects, ${persons.length} persons, ${companiesData.length} companiesData`);

    // Process emails
    for (const email of emailsToProcess) {
      // Check if we've exceeded max processing time
      if (Date.now() - startTime > maxProcessingTime) {
        console.log(`⏰ Max processing time reached (${maxProcessingTime}ms)`);
        break;
      }
    ...
    console.log(`✅ Cloud processing completed in ${processingTime}ms`);
